...
<%
	...
	if(listCount > 0) listSecurity = dao.getListSecurity(startRow, endRow);			
...
<table id="t1">
	<tr>
		<td colspan="8" class="boardTitle"><h3>보안구역 미퇴행 리스트</h3></td>
	</tr>
	<tr>
		<td class="listCount" colspan="8">보안구역 미퇴행 자 : <%=listCount %></td> 
	</tr>
	<tr>
	...
	<tr>
		<td class="listTdSpan" colspan="8">출입자가 존재하지 않습니다.</td>
	</tr>
	</c:if>
	<c:if test="<%= listCount !=0 %>">
	...
	<tr class="listTr">
		...
		<c:if test="${security.securityState==1}"><td>보안구역 출입</td></c:if>
	</tr>
	</c:forEach>
	</c:if>
	<tr>
...
<%
	if(listCount > 0) {
		...
		if(startPage > PAGE_GROUP) {	%>
			...
			<a href="listVisitant.jsp?pageNum=<%= startPage - PAGE_GROUP %>">
				[이전]</a>
<% 	}
		for(int i = startPage; i <= endPage; i++) { 
			...
			if(i == pageInteger) { %>
				...
				<a  class="nonstyle" href="listSecurityVisitant.jsp?pageNum=<%= i %>" class="page active"><%= i %></a>
										
<%		} else { %>
			
				<a  class="nonstyle" href="listSecurityVisitant.jsp?pageNum=<%= i %>" class="page"><%= i %></a>
<%
			}
		}
		...
		if(endPage < pageCount) {%>
			...
			<a href="listSecurityVisitant.jsp?pageNum=<%= startPage + PAGE_GROUP %>">
				[다음]</a>
<%	}
	}	%>		
		</td>			
