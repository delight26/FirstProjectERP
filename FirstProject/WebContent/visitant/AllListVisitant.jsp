...
<%
	...
	if(listCount > 0) visitantList = dao.getAllvisitantList(startRow, endRow);
...
<table id="t1">
	<tr>
		<td colspan="6" class="boardTitle"><h3>정문 출입자 리스트</h3></td>
	</tr>
	<tr>
		<td class="listCount" colspan="6">출입자 : <%=listCount %></td> 
	</tr>
	<tr>
	...
	<tr class="listTr">
		...
		<td class="listNumber">${visitant.visitnumber }		</td>
		...
		<c:if test="${visitant.state==1}"><td style="color: blue;">출입</td></c:if>
...
<%
	if(listCount > 0) {
		...
		if(startPage > PAGE_GROUP) {	%>
			...
			<a href="AllListVisitant.jsp?pageNum=<%= startPage - PAGE_GROUP %>" class="page">
				이전</a>
<% 	}
		for(int i = startPage; i <= endPage; i++) { 
			...
			if(i == pageInteger) { %>
				...
				<a  class="nonstyle" href="AllListVisitant.jsp?pageNum=<%= i %>" class="page active"><%= i %></a>
										
<%		} else { %>
			
				<a  class="nonstyle" href="AllListVisitant.jsp?pageNum=<%= i %>" class="page"><%= i %></a>
<%
			}
		}	
		if(endPage < pageCount) {%>
